...
def run_inference(test_path, model_path, output_csv, vocab_size):
    ...
    with torch.no_grad():
        for inp, tgt in loader:
            ...
            pred = torch.cat([tgt[:, :1], pred], dim=1)  # Get the first token back
    ...
    print(f" The test results have been saved to: {output_csv}")

# ---------------- Run ----------------
run_inference(
    test_path="E:\Language Model Project\\final-model\\Data\\test_15-20.csv",
    model_path="E:\Language Model Project\\final-model\\result\\expanded 10 times\\20epochs\\best_model.pth",
    output_csv="E:\Language Model Project\\final-model\\result\\expanded 10 times\\20epochs\\test_result.csv",
    vocab_size=134606  # vocab size for training
)
